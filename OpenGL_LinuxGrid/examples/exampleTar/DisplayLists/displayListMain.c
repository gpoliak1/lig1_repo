...
{
printf("\ncompile_rotate_list");
  glNewList(rotate_list_id, GL_COMPILE); //LIST A
...
printf("\ncompile_rotate_teapot_list");
  glNewList(rotate_teapot_list_id, GL_COMPILE); //LIST B
...
{
printf("\ncompile_teapot_list");
  glNewList(teapot_list_id, GL_COMPILE); //LIST C --> calls LIST B
  glPushMatrix();
...
{ printf("\nDisplayFunc");
  ...
  glCallList(rotate_list_id);  //CALLS LIST 'A'
  ...
  for (i = 0; i < 3; ++i)
      ...
      for (k = 0; k < 3; ++k)
	{
	  ...
	  glCallList(teapot_list_id); //CALLS LIST 'C' WHICH CALLS 'B'
